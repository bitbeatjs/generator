...
module.exports = class extends Generator {
        ...
        try {
            this.answers = await this.prompt([
                ...
                },
                {
                    ...
                    name: 'localRegistry',
                    message:
                        'Using your local registry config instead of the global npm registry?',
                    default: true, // Default to npm
                },
            ]);

            if (this.answers.localRegistry) {
                this.registry = new URL(
            ...
            const latestGeneratorVersion = `${await this._checkVersion(
                'generator-bitbeat'
            )}`;

            if (packageJson.version < latestGeneratorVersion) {
                spinner.text = chalk.yellow(
                    `There is a new version available! (${chalk.red(
                        latestGeneratorVersion
                    )}) Run ${chalk.blue(
                        'npm remove -g generator-bitbeat'
                    )} and ${chalk.blue(
                        'npm i -g generator-bitbeat'
                    )} to update.`
                );
            } else {
                spinner.text = chalk.green(
                    'You are running the latest version of the generator. :)'
                );
            }
            ...
            Object.assign(
                ...
                await this.prompt([
                    ...
                    },
                    {
                        ...
                        message:
                            'Do you want to generate empty directories? (optional)',
                        default: false,
                    },
                    {
                        type: 'confirm',
                        ...
                        await this.prompt([
                            {
                                ...
                                message:
                                    'Would you like to run the websocket server on the web server port?',
                            },
                        ])
                    );
                }
                    ...
                    await this.prompt([
                        {
                            ...
                            name: 'statusAction',
                            message:
                                'Would you like to add a status action for the servers?',
                        },
                    ])
                );
                    ...
                    await this.prompt([
                        {
                            ...
                            message:
                                'Would you like to add a documentation action for the servers?',
                        },
                    ])
                );
            }
            ...
            https
                .get(this.registry.toString(), (res) => {
                    if (res.statusCode === 200) {
                        ...
                        reject(
                            `Could not fetch any version of package '${name}' from '${this.registry.host}'.`
                        );
                    }
                })
    ...
    async writing() {
        try {
            ...
            spinner.start();
            spinner.text =
                'Generating new package.json with latest versions...';
            ...
            let pkgJson = {
                ...
                license: 'MIT',
                dependencies: {
                    '@bitbeat/core': `^${await this._checkVersion(
                        '@bitbeat/core'
                    )}`,
                },
                devDependencies: {
            ...
            if (this.answers.webServer) {
                ...
                Object.assign(pkgJson.dependencies, {
                    '@bitbeat/web': `^${await this._checkVersion(
                        '@bitbeat/web'
                    )}`,
                });
            }
            ...
            if (this.answers.webSocketServer) {
                ...
                Object.assign(pkgJson.dependencies, {
                    '@bitbeat/websocket': `^${await this._checkVersion(
                        '@bitbeat/websocket'
                    )}`,
                });
            ...
            if (this.answers.typescript) {
                ...
                Object.assign(pkgJson.devDependencies, {
                    '@types/debug': `^${await this._checkVersion(
                        '@types/debug'
                    )}`,
                    '@types/node': `^${await this._checkVersion(
                        '@types/node'
                    )}`,
                    '@types/node-cron': `^${await this._checkVersion(
                        '@types/node-cron'
                    )}`,
                    '@types/pino': `^${await this._checkVersion(
                        '@types/pino'
                    )}`,
                    '@typescript-eslint/eslint-plugin': `^${await this._checkVersion(
                        '@typescript-eslint/eslint-plugin'
                    )}`,
                    '@typescript-eslint/parser': `^${await this._checkVersion(
            ...
            if (this.answers.webServer || this.answers.webSocketServer) {
                ...
                if (this.answers.typescript) {
                    typeScriptBootLines.push(
                        `import { registerBulk } from '@bitbeat/core';`
                    );
                } else {
                    jsBootLines.push(
                        `const { registerBulk } =  require('@bitbeat/core');`
                    );
                }
                ...
                if (this.answers.webServer) {
                    ...
                    if (this.answers.typescript) {
                        typeScriptBootLines.push(
                            `import { ${webServerImports.join(
                                ', '
                            )} } from '@bitbeat/web';`
                        );
                    } else {
                        jsBootLines.push(
                            `const { ${webServerImports.join(
                                ', '
                            )} } = require('@bitbeat/web');`
                        );
                    }
                ...
                if (this.answers.webSocketServer) {
                    ...
                    if (this.answers.typescript) {
                        typeScriptBootLines.push(
                            `import { ${webSocketServerImports.join(
                                ', '
                            )} } from '@bitbeat/websocket';`
                        );
                    } else {
                        jsBootLines.push(
                            `const { ${webSocketServerImports.join(
                                ', '
                            )} } = require('@bitbeat/websocket');`
                        );
                    }
                }
